...
window.onload = function () {
  ...
  setTimeout(() => makeSVG(), timerLength);
  ...
  function makePath() {
    path = new Path();
    path.smooth();
    path.strokeColor = colours[currentColour];
  }

  // create SVG
  function makeSVG() {
    console.log("make svg", currentColour, colours.length);
  ...
  function downloadAsSVG(fileName) {
    ...
    if (!fileName) {
      fileName = `dp-3-colour-${svgCount}.svg`;
    }
    svgCount++;
